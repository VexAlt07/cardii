...
function driveInit() {
  // Charge Google Identity Services
  ...
  // Charge l'API Google (pour Picker)
...
// Ouvre le Picker Google Drive pour choisir un fichier JSON à importer
...
async function pickerCallback(data) {
  ...
  try {
    ...
    const existingIds = new Set(state.folders.map(f => f.id));
    let added = 0;
    imported.folders.forEach(f => { if (!existingIds.has(f.id)) { state.folders.push(f); added++; } });
    saveState();
    render();
    ...
    driveSetStatus('connected');
    showToast('Fichier invalide ou illisible', 'error');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-drive-import').addEventListener('click', driveOpenPicker);
